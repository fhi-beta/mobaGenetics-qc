...
#### Other known issues
- The FID representing familial inference is not updated for Y and mitochondrial variants as well as in the genetic relatedness files. The FIDs from the genotypes from common variants should be used instead. We apologize for the inconvenience.
- The genotypes provided represent different batches with variying levels of quality. Imputation quality metrics are available in a table along with the release, this table should be used and not the _info_ column in the _pvar_ files. That column is automatically generated by plink when merging bathes and does not reflect the overall imputation quality.
- The `sex_check` file provided in this release is generated during quality control. Its 'PED_SEX' columns and 'STATUS' represent sex as available in the original genotyping files and should not be used, even if stated otherwise in the files made available to you. The 'SNP_SEX' column is to be used for genetic sex inference and should be identical to the 'SEX' column in the 'psam' file.
