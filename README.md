...
#### Phasing
THe genotypes of common variants are encoded as phased genotyping probabilities. Phasing is conducted within and across batches, but quality is expected to be lower acros batches. If you do not use phasing information, we recommend recoding the genotypes to reduce file size and increase processing speed.
...
#### Duplicates
The release includes samples that have been genotyped across multiple batches. It is up to the analyst to select the SentrixID linked to a given individual.

#### Sex check
The team who QCed the genotypes does not have access to the sex assigned at birth to participants. The sex encoded in the files is inferred from the genetic data using plink. No samples were excluded due to sex check. We recommend that analysts run a sex check and exclude mismatches when relevant to their analyses.
